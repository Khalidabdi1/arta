...
use crate::parser::{Script, Command, ActionCommand};
...
fn validate_command(
    ...
    match cmd {
        ...
        Command::Container(c) => {
            if let crate::parser::ContainerCommand::Create(create) = c {
...
pub fn has_warnings(errors: &[ScriptValidationError]) -> bool {
    errors.iter().any(|e| e.severity == ValidationSeverity::Warning)
}
