...
impl ContainerManager {
    ...
    pub fn export(&self, name: &str, path: &Path) -> Result<()> {
        let container = self.containers.get(name)
            .ok_or_else(|| ArtaError::ExecutionError(
                format!("Container '{}' does not exist", name)
            ))?;
        ...
        // Write to file
        fs::write(path, script).map_err(|e| ArtaError::IoError(e))?;
...
mod tests {
    ...
    fn test_manager_list() {
        let mut manager = ContainerManager::new();
        manager.create("test1", ContainerOptions::default()).unwrap();
        manager.create("test2", ContainerOptions::default()).unwrap();
