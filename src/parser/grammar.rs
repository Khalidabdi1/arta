...
pub fn parse_script(input: &str) -> Result<Script> {
    let pairs = ArtaParser::parse(Rule::script, input)
        .map_err(|e| ArtaError::ParseError(e.to_string()))?;
...
fn parse_command_inner(pair: pest::iterators::Pair<Rule>) -> Result<Command> {
    // command -> statement
    let statement = pair.into_inner().next()
...
fn parse_statement(pair: pest::iterators::Pair<Rule>) -> Result<Command> {
    ...
    match inner.as_rule() {
        ...
        _ => Err(ArtaError::ParseError(format!("Unexpected rule in statement: {:?}", inner.as_rule()))),
    }
}
...
fn parse_simple_cmd(pair: pest::iterators::Pair<Rule>) -> Result<Command> {
    ...
    match inner.as_rule() {
        ...
        Rule::explain_cmd => {
            let inner_cmd = inner.into_inner().next()
                .ok_or_else(|| ArtaError::ParseError("Expected command after EXPLAIN".to_string()))?;
        ...
        _ => Err(ArtaError::ParseError(format!("Unexpected rule: {:?}", inner.as_rule()))),
    }
}
...
fn parse_life_target(pair: pest::iterators::Pair<Rule>) -> Result<LifeTarget> {
    ...
    match target_str.as_str() {
        ...
        _ => Err(ArtaError::ParseError(format!("Unknown LIFE target: {}", target_str))),
    }
}
...
fn parse_container_cmd(pair: pest::iterators::Pair<Rule>) -> Result<ContainerCommand> {
    ...
    match inner.as_rule() {
        ...
        _ => Err(ArtaError::ParseError(format!("Unknown container command: {:?}", inner.as_rule()))),
    }
}
...
fn parse_create_container(pair: pest::iterators::Pair<Rule>) -> Result<ContainerCommand> {
    ...
    Ok(ContainerCommand::Create(CreateContainer { name, options, body }))
}
...
fn parse_switch_container(pair: pest::iterators::Pair<Rule>) -> Result<ContainerCommand> {
    let name_pair = pair.into_inner().next()
        .ok_or_else(|| ArtaError::ParseError("Expected container name after SWITCH CONTAINER".to_string()))?;
    let name = parse_container_name(name_pair)?;
...
fn parse_destroy_container(pair: pest::iterators::Pair<Rule>) -> Result<ContainerCommand> {
    let name_pair = pair.into_inner().next()
        .ok_or_else(|| ArtaError::ParseError("Expected container name after DESTROY CONTAINER".to_string()))?;
    let name = parse_container_name(name_pair)?;
...
fn parse_export_container(pair: pest::iterators::Pair<Rule>) -> Result<ContainerCommand> {
    ...
    let name_pair = inner.next()
        .ok_or_else(|| ArtaError::ParseError("Expected container name after EXPORT CONTAINER".to_string()))?;
    let name = parse_container_name(name_pair)?;
    ...
    match first.as_rule() {
        Rule::query_target => {
            ...
            let field = inner.next()
                .ok_or_else(|| ArtaError::ParseError("Expected field after query target in PRINT".to_string()))?
                .as_str()
                .to_string();
        ...
        Rule::identifier => {
            Ok(PrintExpr::Variable(first.as_str().to_string()))
        }
        _ => Err(ArtaError::ParseError(format!("Invalid print expression: {:?}", first.as_rule()))),
    }
}
...
fn parse_if_cmd(pair: pest::iterators::Pair<Rule>) -> Result<IfStatement> {
    ...
    let else_body = if let Some(else_pair) = inner.next() {
        if else_pair.as_rule() == Rule::else_clause {
            let else_block = else_pair.into_inner().next()
                .ok_or_else(|| ArtaError::ParseError("Expected statement block in ELSE".to_string()))?;
...
fn parse_if_condition(pair: pest::iterators::Pair<Rule>) -> Result<IfCondition> {
    ...
    // Parse query target
    let target_pair = inner.next()
        .ok_or_else(|| ArtaError::ParseError("Expected query target in IF condition".to_string()))?;
...
fn parse_context_cmd(pair: pest::iterators::Pair<Rule>) -> Result<ContextCommand> {
    ...
    match inner.as_rule() {
        ...
        _ => Err(ArtaError::ParseError(format!("Unknown context command: {:?}", inner.as_rule()))),
    }
}
...
fn parse_enter_cmd(pair: pest::iterators::Pair<Rule>) -> Result<ContextCommand> {
    ...
    match inner.as_rule() {
        Rule::enter_folder => {
            let path_pair = inner.into_inner().next()
                .ok_or_else(|| ArtaError::ParseError("Expected path after ENTER FOLDER".to_string()))?;
            let path = parse_path_value(path_pair)?;
            Ok(ContextCommand::EnterFolder(path))
        }
        Rule::enter_file => {
            let path_pair = inner.into_inner().next()
                .ok_or_else(|| ArtaError::ParseError("Expected path after ENTER FILE".to_string()))?;
            let path = parse_path_value(path_pair)?;
...
fn parse_show_cmd(pair: pest::iterators::Pair<Rule>) -> Result<ContextCommand> {
    ...
    let target = match target_str.as_str() {
        ...
        _ => return Err(ArtaError::ParseError(format!("Unknown SHOW target: {}", target_str))),
    };
...
fn parse_let_value(pair: pest::iterators::Pair<Rule>) -> Result<LetValue> {
    ...
    match inner.as_rule() {
        Rule::path_value => {
            ...
            let path_inner = inner.into_inner().next()
        ...
        _ => Err(ArtaError::ParseError(format!("Invalid LET value type: {:?}", inner.as_rule()))),
    }
}
...
fn parse_where_clause(pair: pest::iterators::Pair<Rule>) -> Result<WhereClause> {
    let condition_expr = pair.into_inner().next()
...
fn parse_condition_expr(pair: pest::iterators::Pair<Rule>) -> Result<ConditionExpr> {
    ...
    while let Some(op_pair) = inner.next() {
        ...
        if let Some(next_cond) = inner.next() {
            ...
            next = Some((logical_op, Box::new(ConditionExpr {
...
fn parse_compare_op(pair: pest::iterators::Pair<Rule>) -> Result<CompareOp> {
    ...
    match op_str.as_str() {
        ...
        _ => Err(ArtaError::ParseError(format!("Unknown operator: {}", op_str))),
    }
}
...
fn parse_delete_cmd(pair: pest::iterators::Pair<Rule>) -> Result<DeleteFilesCommand> {
    ...
    let where_clause = inner.next()
        .map(|p| parse_where_clause(p))
        .transpose()?;
...
fn parse_kill_cmd(pair: pest::iterators::Pair<Rule>) -> Result<KillProcessCommand> {
    let where_pair = pair.into_inner().next()
        .ok_or_else(|| ArtaError::ParseError("Expected WHERE clause in KILL command".to_string()))?;
...
mod tests {
    ...
    fn test_parse_for_loop_basic() {
        let cmd = parse_command("FOR file IN SELECT FILES * FROM /tmp DO SELECT CONTENT * FROM file END FOR").unwrap();
        match cmd {
            Command::For(f) => {
    ...
    fn test_parse_for_loop_multiple_body_statements() {
        let cmd = parse_command("FOR file IN SELECT FILES * FROM /tmp DO LET x = 1; SELECT CPU * END FOR").unwrap();
        match cmd {
            Command::For(f) => {
    ...
    fn test_parse_for_loop_processes() {
        let cmd = parse_command("FOR proc IN SELECT PROCESS * WHERE cpu > 10 DO SELECT MEMORY * END FOR").unwrap();
        match cmd {
            Command::For(f) => {
    ...
    fn test_parse_if_basic() {
        let cmd = parse_command("IF SELECT MEMORY used_percent > 80 THEN SELECT PROCESS * END IF").unwrap();
        match cmd {
            Command::If(i) => {
    ...
    fn test_parse_if_with_else() {
        let cmd = parse_command("IF SELECT CPU usage > 90 THEN SELECT PROCESS * ELSE SELECT SYSTEM * END IF").unwrap();
        match cmd {
            Command::If(i) => {
    ...
    fn test_parse_life_cpu_multiple_statements() {
        let cmd = parse_command("LIFE MONITOR CPU DO PRINT CPU usage; SELECT PROCESS * WHERE cpu > 50 END LIFE").unwrap();
        match cmd {
            Command::Life(l) => {
    ...
    fn test_parse_create_container_basic() {
        let cmd = parse_command("CREATE CONTAINER \"sandbox\" DO SELECT CPU * END CONTAINER").unwrap();
        match cmd {
    ...
    fn test_parse_create_container_identifier_name() {
        let cmd = parse_command("CREATE CONTAINER mycontainer DO SELECT CPU * END CONTAINER").unwrap();
        match cmd {
            Command::Container(ContainerCommand::Create(c)) => {
    ...
    fn test_parse_create_container_multiple_statements() {
        let cmd = parse_command("CREATE CONTAINER \"test\" DO LET x = 1; SELECT CPU *; SELECT MEMORY * END CONTAINER").unwrap();
        match cmd {
            Command::Container(ContainerCommand::Create(c)) => {
